...
const Index = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${backendUrl}/api/checkKycVerification?email=${email}`
        );
        const data = await response.json();
        console.log("Response data:", data);

        // If KYC verification is true, set kycVerified to true
        if (data.kycverification == "true") {
    ...
    const fetchSubmission = async () => {
      try {
        const response = await fetch(
          `${backendUrl}api/getKycSubmission?email=${email}`
        );
        const data = await response.json();
    ...
    <div className="min-h-screen">
      {loading ? (
        ...
        <div className="lg:ml-64">
          {kycVerified ? (
            <div
              className="bg-green-50 text-green-800 pl-4 pr-10 py-4 rounded relative"
              role="alert"
            >
            ...
            <div
              className="bg-yellow-50 text-yellow-800 pl-4 pr-10 py-4 rounded relative"
              role="alert"
            >
              <div className="mb-3 flex items-center">
                <strong className="font-bold text-base mr-3">
              ...
              <span className="block sm:inline text-sm">
                Your KYC verification is under process. It may take up to 3
                business days.
              </span>
            </div>
          ) : (
            ...
            >
              ...
              <span className="block sm:inline text-sm">
                This is a warning message, please verify your KYC. It may take
                up to 3 business days.
              </span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-3.5 cursor-pointer fill-yellow-500 absolute right-4 top-4"
              ...
              <Link href={`/Dashboard/verification?email=${email}`}>
                <h1 className="border-b border-yellow-800 block w-max text-sm text-yellow-800 mt-3">
                  Verify
                </h1>
              </Link>
            </div>
